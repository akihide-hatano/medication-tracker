        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @auth
                        ...
                        <x-nav-link :href="route('posts.calendar')" :active="request()->routeIs('posts.calendar')">
                            {{ __('カレンダー') }}
                        </x-nav-link>
                    @else
                        {{-- 未認証ユーザー向けのリンク（必要であれば。通常は右側のログイン/登録ボタンで十分） --}}
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                ...
                @else
                    ...
                    <div class="space-x-4">
                        <a href="{{ route('login') }}" class="font-medium text-gray-500 hover:text-gray-700">ログイン</a>
                        <a href="{{ route('register') }}" class="font-medium text-gray-500 hover:text-gray-700">新規登録</a>
                    </div>
                @endauth
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @auth
                ...
                <x-responsive-nav-link :href="route('posts.calendar')" :active="request()->routeIs('posts.calendar')">
                    {{ __('カレンダー') }}
                </x-responsive-nav-link>
            @else
                {{-- 未認証ユーザー向けのレスポンシブリンク --}}
